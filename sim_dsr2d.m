...
parfor iRFa = 1 : nRF
    for iRFb = 1 : nRF
        ...
        for iOffset = 1 : nOffset
            ...
            t = 0;
            for iT = 1 : floor(nT/2)
                H = H0 + nu1 * Sy + 2 * nu2 * cos(2 * pi * nuRF(iRFa) * t) * Sz;
                U = expm(-2i * pi * H * dT);
                s = U * s * U';
                t = t + dT;
            end

            for iT = floor(nT/2) + 1 : nT
                H = H0 - nu1 * Sx + 2 * nu2 * cos(2 * pi * nuRF(iRFb) * t) * Sz;
                U = expm(-2i * pi * H * dT);
                s = U * s * U';
                t = t + dT;
            end
            ...
            signal(iRFa, iRFb) = signal(iRFa, iRFb) + weights(iOffset) * trace(M * s);
        end
    end
end
toc
